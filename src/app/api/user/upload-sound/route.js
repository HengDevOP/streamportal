...
export async function POST(request) {
  ...
  try {
    ...
    if (![".mp3", ".wav", ".ogg"].includes(ext)) {
      return NextResponse.json({ error: "Only .mp3, .wav, and .ogg audio formats are supported!" }, { status: 400 });
    }
    ...
    const soundPath = `/uploads/sounds/${finalFilename}`;
    const updatedConfig = {
      ...user.alertConfig,
      soundUrl: soundPath
    };
    ...
    return NextResponse.json({ success: true, soundUrl: soundPath });
  } catch (err) {
    console.error("Audio upload route error:", err);
    return NextResponse.json({ error: "Server error during sound upload" }, { status: 500 });
  }
}
